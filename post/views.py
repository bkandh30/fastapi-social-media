...
from .service import (
    create_post_service,
    delete_post_service,
    create_hashtag_service,
    get_post_from_post_id_service,
    get_posts_from_hashtag_service,
    get_random_posts_service,
...
async def like_post(post_id: int, username: str, db: Session = Depends(get_db)):
    res, detail = await like_post_service(db, post_id, username)
    if res == False:
...
async def like_post(post_id: int, username: str, db: Session = Depends(get_db)):
    res, detail = await unlike_post_service(db, post_id, username)
    if res == False:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=detail)
